...
To evaluate the impact of alignment order, we randomized the order of the 21 B. rapa genomes 100 times and calculated the resulting Pan-Block sizes (Table S20), which ranged from 956.00 Mb to 997.36 Mb (average 978.96 Mb), demonstrating that genome order has only a minor effect on the overall Pan-Block size.
